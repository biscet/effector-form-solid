...
export function createField(
  ...
  const initValue =
    typeof fieldConfig.init === "function"
      ? fieldConfig.init()
      : fieldConfig.init;
...
export function bindValidation(
  ...
  $errors
    .on(validationEvents, (_, { form, fieldValue, rulesSources }) =>
      validator(fieldValue, form, rulesSources)
    )
...
export function bindChangeEvent({ field, form }: BindChangeEventParams): void {
  const {
    $value,
    $initValue,
    $touched,
  ...
  const { setForm, setInitialForm, resetForm, resetTouched, resetValues } =
    form;

  const resetValueWithInit = sample({
    source: $initValue,
    clock: merge([reset, resetValue, resetValues, resetForm]),
  });
  ...
  $initValue.on(setInitialForm, (curr, updateSet) =>
    updateSet.hasOwnProperty(name) ? updateSet[name] : curr
  );

  $value
    .on(changed, (_, value) => value)
    .on([setForm, setInitialForm], (curr, updateSet) =>
      updateSet.hasOwnProperty(name) ? updateSet[name] : curr
    )
