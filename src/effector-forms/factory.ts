import { EventCallable, Store, combine, sample } from "effector";
import {
  AnyFields,
  AnyFormValues,
  FormConfig,
...
function createFormValuesStore(fields: AnyFields): Store<AnyFormValues> {
...
export function createForm<Values extends AnyFormValues>(
  ...
  const $isFormValid = $filter
    ...
    : $eachValid;
  const $isDirty = combine(dirtyFlagsArr).map((dirtyFlags) =>
    dirtyFlags.some(Boolean)
  );
  const $touched = combine(touchedFlagsArr).map((touchedFlags) =>
    touchedFlags.some(Boolean)
  );
